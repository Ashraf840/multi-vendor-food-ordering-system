# Generated by Django 3.2.8 on 2021-11-19 12:00

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='PromoCode',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('promo_code_name', models.CharField(max_length=100, null=True, unique=True)),
                ('promo_code', models.CharField(blank=True, max_length=100, null=True, unique=True)),
                ('discount_price', models.FloatField()),
                ('created_at', models.DateField(auto_now_add=True, verbose_name='Date Created')),
                ('last_updated_at', models.DateField(auto_now_add=True, verbose_name='Last Updated')),
                ('is_available', models.BooleanField(default=True)),
                ('rest_owner', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='rest-owner+', to=settings.AUTH_USER_MODEL, verbose_name='Restaurant Owner')),
            ],
            options={
                'verbose_name_plural': 'Promo Code',
            },
        ),
    ]
